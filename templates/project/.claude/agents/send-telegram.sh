...
# every failure is logged to .claude/logs/tg-send-errors.log and stderr.
#
# Reads token + chat from `.env` in the project workspace root (the
# directory two levels above this script — i.e. projects/<name>/.env
# in a fleet layout, or just .env when invoked standalone).
#
# Usage:
